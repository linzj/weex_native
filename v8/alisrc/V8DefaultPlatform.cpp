...
v8::platform1::V8DefaultPlatform* CreateV8DefaultPlatform(
  ...
  V8DefaultPlatform* platform = new V8DefaultPlatform();
...
void V8DefaultPlatform::SetThreadPoolSize(int thread_pool_size) {
  std::lock_guard<std::mutex> guard(lock_);
  if (thread_pool_size < 1) {
    long result = sysconf(_SC_NPROCESSORS_ONLN);  // NOLINT(runtime/int)
    LOGD("processor count:%d", (int)result);
    if (result == -1) {
      thread_pool_size = 1;
    } else {
      thread_pool_size = static_cast<int>(result);
    }
  }
  thread_pool_size_ =
...
bool V8DefaultPlatform::PumpMessageLoop(v8::Isolate* isolate) {
  Task* task = NULL;
  {
    std::lock_guard<std::mutex> guard(lock_);
...
size_t V8DefaultPlatform::NumberOfAvailableBackgroundThreads() {
...
void V8DefaultPlatform::CallOnForegroundThread(v8::Isolate* isolate,
                                               Task* task) {
  LOGD("V8DefaultPlatform::CallOnForegroundThread");
  std::lock_guard<std::mutex> guard(lock_);
  main_thread_queue_[isolate].push(task);
  if (isolate == NULL || task == NULL) {
    return;
  }
}

void V8DefaultPlatform::CallDelayedOnForegroundThread(Isolate* isolate,
                                                      Task* task,
...
double V8DefaultPlatform::MonotonicallyIncreasingTime() {
  struct timespec ts;
  int64_t ticks;
  if (clock_gettime(CLOCK_MONOTONIC, &ts) != 0) {
    ...
    ticks = (static_cast<int64_t>(ts.tv_sec) * kMicrosecondsPerSecond) +
            (static_cast<int64_t>(ts.tv_nsec / kNanosecondsPerMicrosecond));
  }
