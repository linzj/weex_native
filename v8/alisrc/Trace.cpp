...
void WriteEvent(char phase, const char* category_group, const char* name,
...
static bool isTraceEnabled(JNIEnv* env) {
  ...
  jmethodID mid =
      (env)->GetStaticMethodID(jTraceClass, "getTraceEnabled", "()Z");
...
void TraceEvent::StopATrace(JNIEnv* env) {
  if (jTraceClass != NULL) (env)->DeleteLocalRef(jTraceClass);
...
TraceEvent::TraceEvent(const char* category_group, const char* name,
                       Phase phase)
