...
#define TRACE_EVENT_BEGIN0(category_group, name)                  \
  {                                                               \
    ...
    TraceEvent te(category_group, name, TRACE_EVENT_PHASE_BEGIN); \
    te.SendToATrace();                                            \
  }
...
#define TRACE_EVENT_END0(category_group, name)                  \
  {                                                             \
    using base::debug::TraceEvent;                              \
    TraceEvent te(category_group, name, TRACE_EVENT_PHASE_END); \
    te.SendToATrace();                                          \
  }
...
class TraceScope {
 public:
  inline TraceScope(const char* category_group, const char* name,
...
TraceScope::TraceScope(const char* category_group, const char* name,
                       const char* key, const char* value)
    : category_group_(category_group), name_(name) {
  TRACE_EVENT_BEGIN1(category_group, name, key, value);
}
TraceScope::~TraceScope() { TRACE_EVENT_END0(category_group_, name_); }
}
}
#endif /* TRACE_H */
