...
class Thread::PlatformData {
 public:
  ...
  std::mutex thread_creation_mutex_;
};
...
static void* ThreadEntry(void* arg) {
  ...
  // one).
  {
    std::lock_guard<std::mutex> lock_guard(
        thread->data()->thread_creation_mutex_);
  }
...
void Thread::Start() {
  ...
  {
    std::lock_guard<std::mutex> lock_guard(data_->thread_creation_mutex_);
...
void Thread::Join() { pthread_join(data_->thread_, NULL); }
}

namespace platform1 {

WorkerThread::WorkerThread(V8TaskQueue* queue)
